...
const update = async (req, res) => {
  try {
    ...
    // Update the name in equipment_types AND update all machines that used the old name
...
const remove = async (req, res) => {
  try {
    ...
    const usageRes = await db.query(
      'SELECT COUNT(*) FROM machines WHERE LOWER(eq_type) = LOWER($1) AND active = true',
      [name]
    );
