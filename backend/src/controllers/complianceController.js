  ...
  try {
    const { rows } = await db.query(`
      SELECT m.id AS machine_id, m.slno, m.reg_no, m.eq_type, m.capacity, m.ownership,
             ...
             (mc.attachment_name IS NOT NULL) AS has_attachment,
  ...
  try {
    const { rows } = await db.query(
      `SELECT id, machine_id, doc_type, doc_label, doc_no, issued_date, expiry_date,
              issued_by, notes, attachment_name, attachment_mime, created_at, updated_at
...
// GET /compliance/:id/attachment — stream attachment file to client
async function getAttachment(req, res) {
  const { id } = req.params
  try {
    const { rows } = await db.query(
      'SELECT attachment_name, attachment_data, attachment_mime FROM machine_compliance WHERE id = $1',
      [id]
    )
    if (!rows.length || !rows[0].attachment_data)
      return res.status(404).json({ error: 'No attachment found' })
    const { attachment_name, attachment_data, attachment_mime } = rows[0]
    const buffer = Buffer.from(attachment_data, 'base64')
    res.setHeader('Content-Disposition', `attachment; filename="${encodeURIComponent(attachment_name)}"`)
...
// Attachment handling:
//   - attachment_data present  → update attachment
//   - attachment_data absent   → keep existing (COALESCE)
//   - clear_attachment: true   → remove attachment after upsert
      ...
      const { rows } = await client.query(`
        INSERT INTO machine_compliance
          (machine_id, doc_type, doc_label, doc_no, issued_date, expiry_date, issued_by, notes,
           attachment_name, attachment_data, attachment_mime, updated_at)
        ...
        ON CONFLICT (machine_id, doc_type, doc_label) DO UPDATE SET
          ...
          attachment_data = COALESCE(EXCLUDED.attachment_data, machine_compliance.attachment_data),
          attachment_mime = COALESCE(EXCLUDED.attachment_mime, machine_compliance.attachment_mime),
          updated_at  = NOW()
        RETURNING id, doc_type, doc_label, expiry_date, attachment_name, attachment_mime
      `, [
        machine_id, doc_type, doc_label,
        doc_no || null, issued_date || null, expiry_date || null, issued_by || null, notes || null,
        attachment_name || null, attachment_data || null, attachment_mime || null,
      ])

      // Explicit clear requested: remove attachment after upsert
      if (clear_attachment && rows[0]?.id) {
        await client.query(
          `UPDATE machine_compliance SET attachment_name=NULL, attachment_data=NULL, attachment_mime=NULL WHERE id=$1`,
          [rows[0].id]
        )
