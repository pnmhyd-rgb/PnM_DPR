          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                {['#','Project','SL No','Type','Manufacturer','Model','Capacity / UOM','Reg No','Chassis No','Fuel Type','Shift','Meter','Fuel Min','Fuel Max','Planned Hrs/Day','Purchase Date','Price (₹)'].map(h => (
                  <th key={h} className="px-3 py-2.5 text-left font-semibold text-gray-500 whitespace-nowrap">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {loading && <tr><td colSpan={17} className="px-4 py-10 text-center text-gray-400">Loading…</td></tr>}
              {!loading && machines.length === 0 && <tr><td colSpan={17} className="px-4 py-10 text-center text-gray-400">No measurable own assets found</td></tr>}
