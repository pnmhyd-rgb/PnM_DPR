...
import { Plus, Trash2, Pencil, Check, X, Search, List, AlertTriangle } from 'lucide-react'
...
export default function EquipmentTypes() {
  ...
  const [forceConfirm, setForceConfirm] = useState(null) // { id, name, count }
  ...
  const handleDeleteSelected = async () => {
    ...
    const inUse = ids.filter(id => {
      ...
      return t && parseInt(t.usage_count) > 0
    })
    if (inUse.length > 0) {
      const names = inUse.map(id => types.find(t => t.id === id)?.name).join(', ')
      if (!confirm(`${inUse.length} type(s) are currently assigned to machines: ${names}.\n\nDeleting will NOT remove the machines — their equipment type field will simply no longer match a known type.\n\nContinue?`)) return
    } else if (!confirm(`Delete ${ids.length} equipment type${ids.length > 1 ? 's' : ''}?`)) return
    ...
    <div className="max-w-lg space-y-4">
      ...
      {showBulk && (
        <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
          ...
          <p className="text-xs text-gray-500">Enter one type per line, or separate with commas.</p>
          <textarea
        ...
        <div className="divide-y divide-gray-100 max-h-[480px] overflow-y-auto">
          ...
          {filtered.map(t => (
            ...
            >
              ...
              {editId === t.id ? (
                <div className="flex-1 flex items-center gap-2 min-w-0">
                  <input
                    ...
                    className="flex-1 border border-blue-400 rounded px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 min-w-0"
                  />
              ...
              {/* Action buttons — only when not editing this row */}
              {editId !== t.id && (
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 mt-1">
                  <strong>"{forceConfirm.name}"</strong> is currently assigned to{' '}
                  <strong>{forceConfirm.count} active machine{forceConfirm.count > 1 ? 's' : ''}</strong>.
                  Deleting it will not remove those machines — their equipment type field will just no longer match a known type.
                </p>
              </div>
            </div>
            <div className="flex gap-3">
